 ...
 * consults only the claim flag, so without this a self-hosted instance gets an
 * unbranded signing page but a branded email footer — the two paths disagree.
 *
 * Note upstream already applies the same `!IS_BILLING_ENABLED()` reasoning to
 * `allowBrandedEmailColors` below; this extends it to the powered-by line.
 */
const resolveHidePoweredBy = (claimFlag: boolean | undefined) =>
  !IS_BILLING_ENABLED() || claimFlag === true;
...
const handleTeamEmailContext = async (teamId: number) => {
  ...
  const branding = teamGlobalSettingsToBranding(teamSettings, teamId, resolveHidePoweredBy(claims.flags.hidePoweredBy));
